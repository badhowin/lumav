...
export default class AddProductForm extends Component {
  ...
  isNameOk = () => {
    ...
    this.setState({ productNameWrong: "wrong" });
  ...
  isPriceOk = () => {
    if (
      !isNaN(this.state.productCard.productPrice) &&
      this.state.productCard.productPrice.length
    ) {
      return true;
    }
    this.setState({ productPriceWrong: "wrong" });
    return false;
  };
  ...
  isPicFileOk = () => {
    ...
    this.setState({ picFileWrong: "wrong" });
    return false;
  };

  render() {
    const productNameClass =
      "form-name-field form-input " + this.state.productNameWrong;
    const productPriceClass =
      "form-price-field form-input " + this.state.productPriceWrong;
    const picFileClass =
      "form-file-input form-input " + this.state.picFileWrong;

    return (
      <div className="add-product-form">
        <form onSubmit={this.onSubmit}>
          <div className={picFileClass} onClick={this.choosePic}>
            {this.state.fileInputText}
          </div>

          <input
            onChange={this.imgInputChanged}
            id="img-input"
            type="file"
            name="photo"
            accept="image/*"
